...
public class BufferedIOBenchmark {
   ...
   public static void main(String[] args) throws IOException {
	  ...
	  FileOutputStream fos = new FileOutputStream("metrics.csv");
	...
	*/
   private void produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) throws IOException {
	  LOG.log(Level.INFO, "Generating test data ({0}, {1} bytes, block size: {2}...",
			  new Object[]{ioStrategy, numberOfBytesToWrite, blockSize});
	  Timer.start();
	  ...
	  try {
		 ...
		 if ((ioStrategy == IOStrategy.BlockByBlockWithBufferedStream) ||
			 (ioStrategy == IOStrategy.ByteByByteWithBufferedStream)) {
	...
	*/
   private void produceDataToStream(OutputStream os, IOStrategy ioStrategy, long numberOfBytesToWrite,
	  ...
	  if ((ioStrategy == IOStrategy.ByteByByteWithBufferedStream) ||
		  (ioStrategy == IOStrategy.ByteByByteWithoutBufferedStream)) {
...
/**
 * This class represent a specific CSV for this lab.
 */
class CSV {
   private OutputStreamWriter os; // The stream we will write in

   public CSV(OutputStream os) throws IOException {
	  this.os = new OutputStreamWriter(os);
	  this.os.write("operation,strategy,blockSize,fileSizeInBytes,durationInMs\n");
   }

   /**
	* Format and write for the CSV
	*
	* @param operation
	* @param strategy
	* @param blockSize
   ...
   /**
	* This enum is used to describe the 2 different operation that we can do on a file
	*/
